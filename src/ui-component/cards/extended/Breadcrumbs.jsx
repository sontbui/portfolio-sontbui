  ...
  if ((item && item.type === 'item') || (item?.type === 'group' && item?.url) || custom) {
    ...
    let tempContent = (
      ...
      >
        <Typography 
        // component={Link} to='/'
         color='textSecondary' variant='subtitle1' sx={linkSX}>
          {icons && <HomeTwoToneIcon style={iconSX} />}
          {icon && !icons && <HomeIcon style={{ ...iconSX, marginRight: 0 }} />}
          {(!icon || icons) && 'Dashboard'}
        </Typography>
    ...
    if (item?.breadcrumbs !== false || custom) {
      breadcrumbContent = (
        <Card
          ...
          {...others}
        >
          <Box sx={{ p: 2, pl: card === false ? 0 : 2, display: { xs: 'none', md: 'block' } }}>
            <Grid
              container
              direction={rightAlign ? 'row' : 'column'}
              justifyContent={rightAlign ? 'space-between' : 'flex-start'}
            ...
            >
              {/* content start */}
              {/* {title && !titleBottom && <BTitle title={custom ? heading : t(`${item?.title}`)} />} */}
